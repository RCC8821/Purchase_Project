...
const Take_Quotation = () => {
  ...
  const addMaterialToVendor = (vendorIndex) => {
    ...
    newVendors[vendorIndex].materials.push({
      ...
      "IGST (%)": "",
      Total: "",
      Brand_Name: "", // Renamed from Remark
      Revised_Quantity: "",
      Unit_Name: "",
      "DECIDED_BRAND/COMPANY_NAME_2": "",
      Total_Value: "",
    ...
    for (const vendor of vendors) {
      for (const material of vendor.materials) {
        ...
        entries.push({
          ...
          PLANNED_4: status4,
          NO_OF_QUOTATION_4: noOfQuotation4,
          REMARK_4: material.Brand_Name || "", // Now sending material-level Brand Name
          REVISED_QUANTITY_2: material.Revised_Quantity,
          Total_Value: totalValue,
        });
      }
              ...
              <div className="px-6 py-6">
                                  ...
                                  >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                      ...
                                      <div className="md:col-span-2">
                                        ...
                                        <input
                                          value={material.Brand_Name || ""}
                                          onChange={(e) =>
                                            handleMaterialChange(vIndex, mIndex, "Brand_Name", e.target.value)
                                          }
                                          className="w-full p-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-sm"
                                          placeholder="Enter quoted brand name"
                                          disabled={isSaving}
                                        />
                                      </div>
                                    </div>
                ...
                {currentStep === 4 && (
                  <div className="space-y-6">
                    ...
                    {/* Review content same as before, only Brand Name field updated */}
                    <div className="space-y-6">
                      {vendors.map((vendor, vIndex) => (
                        <div key={vIndex} className="bg-gray-50 border border-gray-200 rounded-xl p-4 shadow-sm">
                          <h6 className="text-md font-semibold text-blue-800 mb-3 border-b border-blue-200 pb-2">
                            Vendor {vIndex + 1}: {vendor.firm}
                          </h6>
                          {/* Vendor details... */}
                            ...
                            <div className="space-y-3">
                              {vendor.materials.map((material, mIndex) => (
                                <div key={mIndex} className="bg-white border border-gray-100 rounded-lg p-3 shadow-sm">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                                    <div><label className="text-gray-500">Material Name</label><p>{material.Material_Name}</p></div>
                                    <div><label className="text-gray-500">Rate</label><p>{material.Rate}</p></div>
                                    <div><label className="text-gray-500">Total (After Tax)</label><p className="font-semibold text-green-600">{material.Total}</p></div>
                                    <div><label className="text-gray-500">Total Value</label><p className="font-semibold text-blue-600">{material.Total_Value}</p></div>
                                    <div className="md:col-span-2">
                                      <label className="text-gray-500">Brand Name</label>
                                      <p>{material.Brand_Name || "-"}</p>
                                    </div>
                                  </div>
                                </div>
                              ))}
                ...
                {currentStep === 5 && (
                  <div className="space-y-6">
                    <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 mb-6">
                      ...
                      <p className="text-orange-700 text-sm">
                        Number of quotations is auto-set from vendors. Brand name is taken from material entries.
                      </p>
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-3">
                        Status 4 <span className="text-red-500">*</span>
                      </label>
                      <select
                        value={status4}
                        onChange={(e) => setStatus4(e.target.value)}
                        className="w-full p-4 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200 bg-white cursor-pointer shadow-sm hover:shadow-md"
                        disabled={isSaving}
                      >
                        <option value="Done">Done</option>
                        <option value="PENDING">PENDING</option>
              ...
              <div className="flex justify-between">
                ...
                {currentStep < 5 && (
                  <button
                    ...
                    className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 font-medium shadow-lg hover:shadow-xl ml-auto"
                    disabled={
                      (currentStep === 1 && !selectedIndent) ||
                      (currentStep === 3 && !validateStep3()) ||
                      isSaving
                ...
                {currentStep === 5 && (
                  <button
                    onClick={handleSave}
                    className="px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-200 font-medium flex items-center justify-center min-w-[140px] shadow-lg hover:shadow-xl ml-auto"
                    disabled={isSaving}
                  >
                    {isSaving ? (
                      <>
