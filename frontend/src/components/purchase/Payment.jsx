...
const fetchInitialData = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/Payment`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = selectedBills.map(uid => {
        ...
        const netAmount = Number(bill.netAmount16 || 0);
        const previousPaid = Number(bill.latestPaidAmount || 0);
        const totalPaidAfter = previousPaid + currentPaid;  // Cumulative for FMS
        ...
        return {
          ...
          currentPaid: currentPaid,      // ← Yeh sirf Payment_Sheet ke liye
          paidAmount17: totalPaidAfter,             // ← Cumulative for Billing_FMS
      ...
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/Update-Payment`, payload);
      if (response.data.success) {
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-end' }}>
          <div style={{ flex: 1, position: 'relative' }} ref={vendorRef}>
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <input type="text" placeholder="Search Vendor..." value={vendorSearch} onFocus={() => setShowVendorList(true)} onChange={(e) => setVendorSearch(e.target.value)} style={{ width: '100%', padding: '12px 15px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '16px', outline: 'none' }} />
            ...
            {showVendorList && (
              <ul style={{ position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: 'white', border: '1px solid #ddd', borderRadius: '8px', marginTop: '5px', maxHeight: '200px', overflowY: 'auto', zIndex: 100, listStyle: 'none', padding: 0, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                {vendors.filter(v => v.vendorFirmName.toLowerCase().includes(vendorSearch.toLowerCase())).map((v, i) => (
                  <li key={i} onClick={() => { setVendorSearch(v.vendorFirmName); setShowVendorList(false); }} style={{ padding: '10px 15px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0' }} onMouseOver={(e) => e.target.style.backgroundColor = '#f0f7ff'} onMouseOut={(e) => e.target.style.backgroundColor = 'transparent'}>
                    {v.vendorFirmName}
                  </li>
                ))}
              </ul>
          ...
          <div style={{ flex: 1, position: 'relative' }} ref={siteRef}>
            ...
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <input type="text" placeholder="Search Site..." value={siteSearch} onFocus={() => setShowSiteList(true)} onChange={(e) => setSiteSearch(e.target.value)} style={{ width: '100%', padding: '12px 15px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '16px', outline: 'none' }} />
              <FaChevronDown style={{ position: 'absolute', right: '15px', color: '#888' }} />
            </div>
            {showSiteList && (
              <ul style={{ position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: 'white', border: '1px solid #ddd', borderRadius: '8px', marginTop: '5px', maxHeight: '200px', overflowY: 'auto', zIndex: 100, listStyle: 'none', padding: 0, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                {sites.filter(s => s.siteName.toLowerCase().includes(siteSearch.toLowerCase())).map((s, i) => (
                  <li key={i} onClick={() => { setSiteSearch(s.siteName); setShowSiteList(false); }} style={{ padding: '10px 15px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0' }} onMouseOver={(e) => e.target.style.backgroundColor = '#f0f7ff'} onMouseOut={(e) => e.target.style.backgroundColor = 'transparent'}>
                    {s.siteName}
                  </li>
                ))}
              </ul>
          ...
          <button onClick={handleFilter} style={{ padding: '12px 35px', backgroundColor: '#d32f2f', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px' }}>
      ...
      {showData && (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <div style={{ color: '#d32f2f', fontWeight: 'bold', cursor: 'pointer', fontSize: '16px' }} onClick={() => { setShowData(false); setSelectedBills([]); setPaidAmounts({}); }}>
          ...
          {filteredData.map((item) => {
            ...
            return (
              <div key={item.UID} style={{ backgroundColor: 'white', borderRadius: '12px', padding: '25px', marginBottom: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: selectedBills.includes(item.UID) ? '10px solid #2e7d32' : '1px solid #eee' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                  <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                    <div onClick={() => toggleBillSelection(item.UID)} style={{ border: '2px solid #ddd', borderRadius: '8px', padding: '10px 15px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: selectedBills.includes(item.UID) ? '#e8f5e9' : 'white' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: '12px', color: '#999' }}>Planned Date: {item.planned17}</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', borderTop: '1px solid #f0f0f0', paddingTop: '20px' }}>
                  ...
                  <DetailItem label="Previous Paid Amount" value={`₹${summary.previousPaid.toLocaleString('en-IN')}`} color='green' />
                  <DetailItem label="Remaining Amount" value={`₹${summary.previousPaid == 0 ? 0 : summary.currentBalance.toLocaleString('en-IN')}`} color='green' />
                </div>
                  ...
                  <div style={{ marginTop: '30px', padding: '20px', backgroundColor: '#f8fff8', borderRadius: '10px', border: '1px solid #c8e6c9' }}>
                    ...
                    <div style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#e3f2fd', borderRadius: '8px', borderLeft: '4px solid #1976d2' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', fontSize: '14px' }}>
                        {/* ← Label NET AMOUNT 17 */}
                        <div><span style={{ color: '#555', fontWeight: 'bold' }}>Original Net Amount (17):</span> <span style={{ marginLeft: '10px', fontWeight: 'bold', color: '#1976d2', fontSize: '16px' }}>₹{summary.netAmount.toLocaleString('en-IN')}</span></div>
                        <div><span style={{ color: '#555', fontWeight: 'bold' }}>Previously Paid:</span> <span style={{ marginLeft: '10px', fontWeight: 'bold', color: '#388e3c', fontSize: '16px' }}>₹{summary.previousPaid.toLocaleString('en-IN')}</span></div>
                        <div><span style={{ color: '#555', fontWeight: 'bold' }}>Current Balance:</span> <span style={{ marginLeft: '10px', fontWeight: 'bold', color: '#d32f2f', fontSize: '16px' }}>₹{summary.currentBalance.toLocaleString('en-IN')}</span></div>
                        <div><span style={{ color: '#555', fontWeight: 'bold' }}>Total Paid After:</span> <span style={{ marginLeft: '10px', fontWeight: 'bold', color: '#388e3c', fontSize: '16px' }}>₹{summary.totalPaidAfter.toLocaleString('en-IN')}</span></div>
                      </div>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
                      <div>
                        <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block' }}>CURRENT PAID AMOUNT ₹</label>
                        <input type="number" value={paidAmounts[item.UID] || ''} onChange={(e) => handlePaidAmountChange(item.UID, e.target.value)} placeholder="Enter amount" min="0" max={summary.currentBalance} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #4caf50', marginTop: '8px' }} />
                      </div>
                      <div>
                        <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block' }}>NEW BALANCE</label>
                        <input type="number" value={summary.newBalance} readOnly style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #d32f2f', backgroundColor: '#ffebee', marginTop: '8px', fontWeight: 'bold' }} />
                      </div>
                    </div>

                    <div style={{ marginTop: '20px' }}>
                      <div style={{ height: '8px', backgroundColor: '#e0e0e0', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ height: '100%', backgroundColor: '#2196f3', width: `${(summary.totalPaidAfter / summary.netAmount) * 100}%`, transition: 'width 0.3s ease' }} />
                      </div>
                    </div>

                    <button onClick={scrollToPaymentSection} style={{ marginTop: '20px', padding: '12px 25px', backgroundColor: '#1976d2', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {selectedBills.length > 0 && (
            <div style={{ backgroundColor: '#e8f5e9', padding: '20px', borderRadius: '12px', marginTop: '30px', border: '2px solid #4caf50' }}>
              <h3 style={{ margin: '0 0 10px 0', color: '#2e7d32' }}>
                ...
                <span style={{ float: 'right', fontSize: '28px', fontWeight: 'bold' }}>₹{grandTotal.toLocaleString('en-IN')}</span>
              </h3>
            </div>
          )}
            ...
            <div ref={paymentSectionRef} style={{ backgroundColor: 'white', padding: '30px', borderRadius: '12px', marginTop: '30px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
                <div>
                  ...
                  <select value={bankDetails} onChange={(e) => setBankDetails(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '8px' }}>
                ...
                <div>
                  ...
                  <select value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '8px' }}>
                    <option value="">-- Select --</option>
                ...
                <div>
                  ...
                  <input type="text" placeholder={paymentMode === "Cheque" ? "Enter Cheque Number" : "Enter detail"} value={paymentDetails} onChange={(e) => setPaymentDetails(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '8px' }} />
                </div>
                <div>
                  <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block' }}>PAYMENT DATE</label>
                  <input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #ddd', marginTop: '8px' }} />
                </div>
              </div>
              ...
              <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#fff3cd', borderRadius: '8px', border: '1px solid #ffeaa7' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ color: '#856404' }}>First payment = Full amount • Next payments deduct from balance</span>
                </div>
              </div>

              <button disabled={submitting} style={{ padding: '15px 40px', backgroundColor: submitting ? '#999' : '#2e7d32', color: 'white', border: 'none', borderRadius: '8px', cursor: submitting ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '18px', marginTop: '30px', opacity: submitting ? 0.7 : 1 }} onClick={handleSubmit}>
...
const DocLink = ({ icon, label, url }) => (
  <a href={url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', padding: '6px 12px', borderRadius: '4px', border: '1px solid #1a73e8', color: '#1a73e8', fontWeight: 'bold', backgroundColor: '#f0f7ff' }}>
    {icon} {label}
  </a>
);
