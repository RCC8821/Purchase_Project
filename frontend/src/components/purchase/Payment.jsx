...
const Payment = () => {
  ...
  const calculateTotalPaidAfter = (bill) => {
    ...
    const currentPaid = paidAmounts[bill.UID] || 0;
    const roundup = roundups[bill.UID] || 0;
    return previousPaid + currentPaid + roundup;
  };

  const grandTotal = selectedBills.reduce(
    (sum, uid) =>
      sum + (paidAmounts[uid] || 0) + (roundups[uid] || 0),
    0
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = selectedBills.map((uid) => {
        ...
        const netAmount = Number(bill.netAmount16 || 0);
        const previousPaid = Number(bill.latestPaidAmount || 0);
        const totalPaidAfter = previousPaid + effectiveCurrentPaid;
        const newBalance = netAmount - totalPaidAfter;
  ...
  const getPaymentSummary = (bill) => {
    ...
    const totalPaidAfter = previousPaid + effectivePaid;
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleRoundupChange(item.UID, e.target.value)
                          }
