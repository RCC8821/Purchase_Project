...
//             // Balance = netAmount - latestPaid (from Payment_Sheet) - advanceAmount (from Advance_Payment_Sheet)
//             latestBalanceAmount:
//               latestPaid > 0
//                 ? netAmount - latestPaid - advanceAmount
...
//         setRoundups((current) => ({ ...current, [uid]: 0 }));
//         return [...prev, uid];
//       }
//     });
//   };
...
//       alert(
//         `Cannot pay more than current balance (₹${currentBalance.toLocaleString("en-IN")})`
//       );
...
//   // Current balance = netAmount - previousPaid(Payment_Sheet) - advanceAmount(Advance_Payment_Sheet)
...
//     const alreadyDeducted = previousPaid + advanceAmount;
//     return alreadyDeducted > 0 ? netAmount - alreadyDeducted : netAmount;
//   };
...
//     const currentBalance = calculateCurrentBalance(bill);
//     return currentBalance - effectivePaid;
//   };
...
//     const currentPaid = paidAmounts[bill.UID] || 0;
//     const roundup = roundups[bill.UID] || 0;
//     return previousPaid + advanceAmount + currentPaid + roundup;
//   };

//   const grandTotal = selectedBills.reduce(
//     (sum, uid) => sum + (paidAmounts[uid] || 0) + (roundups[uid] || 0),
//     0
//   );
...
//       alert(
//         `ध्यान दें!\n${emptyPaidBills.length} बिल(s) में Paid Amount या Round Off खाली है।\n\nसभी बिल्स में payment amount और round off भरें तभी submit हो सकेगा।`
//       );
//       return;
//     }
...
//         `${zeroPaidBills.length} बिल(s) में effective paid amount 0 है। क्या आप submit करना चाहते हैं?`
//       );
...
//         const effectiveCurrentPaid = currentPaid + roundup;
//         const netAmount = Number(bill.netAmount16 || 0);
//         const previousPaid = Number(bill.latestPaidAmount || 0);
//         const advanceAmount = Number(bill.advanceAmount || 0);
//         const totalPaidAfter = previousPaid + advanceAmount + effectiveCurrentPaid;
...
//           netAmount16: netAmount,
//           currentPaid: effectiveCurrentPaid,
...
//     const roundup = roundups[bill.UID] || 0;
//     const effectivePaid = currentPaid + roundup;
//     const newBalance = currentBalance - effectivePaid;
//     const totalPaidAfter = previousPaid + advanceAmount + effectivePaid;
...
//                         alignItems: "center",
//                         gap: "10px",
//                         backgroundColor: selectedBills.includes(item.UID) ? "#e8f5e9" : "white",
//                       }}
//                     >
...
//                         Invoice No:{" "}
//                         <b style={{ backgroundColor: "#f0f7ff", padding: "2px 6px", color: "#1a73e8" }}>
//                           {item.invoice13 || "-"}
//                         </b>
...
//                     color="green"
//                   />
//                   {/* ← Advance Payment separately shown */}
//                   {summary.advanceAmount > 0 && (
//                     <DetailItem
//                       label="Advance Payment (Adjusted)"
//                       value={`₹${summary.advanceAmount.toLocaleString("en-IN")}`}
//                       color="#e65100"
//                     />
//                   )}
//                   <DetailItem
...
//                     color={summary.currentBalance <= 0 ? "#d32f2f" : "green"}
//                   />
//                 </div>

//                 {/* Advance Payment Banner - only show if advance exists */}
//                 {summary.advanceAmount > 0 && (
//                   <div
//                     style={{
//                       marginTop: "15px",
...
//                       Advance Payment of ₹{summary.advanceAmount.toLocaleString("en-IN")} already adjusted against this bill.
//                     </span>
//                   </div>
//                 )}
...
//                 >
//                   {item.finalIndentPDF && (
//                     <DocLink icon={<FaFilePdf />} label="Indent PDF" url={item.finalIndentPDF} />
//                   )}
//                   {item.poPDF && (
//                     <DocLink icon={<FaFilePdf />} label="PO PDF" url={item.poPDF} />
//                   )}
//                   {item.mrnPDF && (
//                     <DocLink icon={<FaFilePdf />} label="MRN PDF" url={item.mrnPDF} />
//                   )}
//                   {item.invoicePhoto && (
//                     <DocLink icon={<FaFilePdf />} label="Invoice Photo PDF" url={item.invoicePhoto} />
//                   )}
//                 </div>
...
//                       >
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Original Net Amount (17):</span>{" "}
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#1976d2", fontSize: "16px" }}>
//                             ₹{summary.netAmount.toLocaleString("en-IN")}
//                           </span>
//                         </div>
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Previously Paid:</span>{" "}
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#388e3c", fontSize: "16px" }}>
//                             ₹{summary.previousPaid.toLocaleString("en-IN")}
//                           </span>
//                         </div>
//                         {/* ← Advance row in payment summary */}
//                         {summary.advanceAmount > 0 && (
//                           <div>
//                             <span style={{ color: "#555", fontWeight: "bold" }}>Advance Adjusted:</span>{" "}
//                             <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#e65100", fontSize: "16px" }}>
//                               ₹{summary.advanceAmount.toLocaleString("en-IN")}
//                             </span>
//                           </div>
//                         )}
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Current Balance:</span>{" "}
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#d32f2f", fontSize: "16px" }}>
//                             ₹{summary.currentBalance.toLocaleString("en-IN")}
//                           </span>
//                         </div>
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Total Paid After:</span>{" "}
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#388e3c", fontSize: "16px" }}>
...
//                     >
//                       <div>
//                         <label style={{ fontSize: "13px", fontWeight: "bold", color: "#555", display: "block" }}>
//                           CURRENT PAID AMOUNT ₹
//                         </label>
//                         <input
//                           type="number"
...
//                       }}
//                     >
//                       <FaArrowDown /> Go to Global Payment Section (Fill Bank, Mode, Date)
//                     </button>
//                   </div>
//                 )}
//               </div>
...
//                 <span style={{ float: "right", fontSize: "28px", fontWeight: "bold" }}>
...
//                   <span style={{ fontWeight: "bold", color: "#856404" }}>⚠️ How it works:</span>
//                   <span style={{ color: "#856404" }}>
//                     Advance payment already adjusted • First payment = Full amount • Next payments deduct from balance
//                   </span>
//                 </div>
//               </div>
...
//   <div>
//     <div style={{ fontSize: "10px", color: "#aaa", fontWeight: "bold", marginBottom: "4px" }}>
//       {label}
//     </div>
...
//             latestPaidAmount:     latestPaid,
//             advanceAmount:        advanceAmount,
//             // Balance = netAmount - previousPaid(H) - advance(P)
...
//     setSelectedBills((prev) => {
//       if (prev.includes(uid)) {
//         setPaidAmounts((current) => { const n = { ...current }; delete n[uid]; return n; });
//         setRoundups((current)    => { const n = { ...current }; delete n[uid]; return n; });
//         return prev.filter((id) => id !== uid);
//       } else {
//         setPaidAmounts((current) => ({ ...current, [uid]: 0 }));
//         setRoundups((current)    => ({ ...current, [uid]: 0 }));
...
//   // currentBalance = netAmount - previousPaid(H column) - advance(P column)
//   const calculateCurrentBalance = (bill) => {
//     const netAmount     = Number(bill.netAmount16    || 0);
//     const previousPaid  = Number(bill.latestPaidAmount || 0);
//     const advanceAmount = Number(bill.advanceAmount   || 0);
...
//     const currentPaid   = paidAmounts[bill.UID] || 0;
//     const roundup       = roundups[bill.UID]    || 0;
//     const effectivePaid = currentPaid + roundup;  // sirf input box value

//     // I column = 0 agar H + P = netAmount, warna remaining
...
//     }
//     if (!bankDetails || !paymentMode || !paymentDetails.trim() || !paymentDate) {
//       alert("Please fill all payment details: Bank, Mode, Details, Date");
//       return;
//     }
...
//       const round = roundups[uid];
//       return (
//         paid === undefined || paid === null || paid === "" || String(paid).trim() === "" || isNaN(paid) ||
//         round === undefined || round === null || round === "" || String(round).trim() === "" || isNaN(round)
//       );
//     });

//     if (emptyPaidBills.length > 0) {
//       alert(`${emptyPaidBills.length} bill(s) mein Paid Amount ya Round Off khali hai. Sab fill karo.`);
//       return;
//     }
...
//     if (zeroPaidBills.length > 0) {
//       const confirmZero = window.confirm(
//         `${zeroPaidBills.length} bill(s) mein effective paid amount 0 hai. Submit karna chahte hain?`
//       );
//       if (!confirmZero) return;
//     }
...
//         const previousPaid  = summary.previousPaid;
//         const advanceAmt    = summary.advanceAmount;
//         const effectivePaid = summary.effectivePaid; // sirf input box value (roundup included)

//         // H column = currentPaid (sirf input, advance nahi)
//         const hColumnValue  = effectivePaid;

//         // BW (Billing_FMS total paid) = previousPaid + currentPaid only (advance alag track hai)
//         const paidAmount17  = previousPaid + effectivePaid;

//         // I column balance = 0 if fully paid, else remaining
//         const totalDeducted = previousPaid + advanceAmt + effectivePaid;
//         const balanceAmount17 = totalDeducted >= netAmount ? 0 : netAmount - totalDeducted;
...
//           netAmount16:      netAmount,
//           currentPaid:      hColumnValue,          // H - input only
//           paidAmount17:     paidAmount17,           // BW in Billing_FMS
//           balanceAmount17:  balanceAmount17,        // I + BX
//           bankDetails17:    bankDetails,
//           paymentMode17:    paymentMode,
//           paymentDetails17: paymentDetails,
//           paymentDate18:    paymentDate,
//           grandTotal:       grandTotal,
//           advanceAmount:    advanceAmt,             // P column
//         };
//       });
...
//                     </div>
//                     <div style={{ fontSize: "12px", color: "#999" }}>Planned: {item.planned17}</div>
//                   </div>
//                 </div>
...
//                   <DetailItem label="MRN NO"             value={item.mrnNo} />
//                   <DetailItem label="PLANNED 17"         value={item.planned17} />
//                   <DetailItem label="NET AMOUNT 17"      value={`Rs.${summary.netAmount.toLocaleString("en-IN")}`} />
//                   <DetailItem label="Previous Paid (H)"  value={`Rs.${summary.previousPaid.toLocaleString("en-IN")}`} color="green" />
//                   {summary.advanceAmount > 0 && (
//                     <DetailItem label="Advance Adjusted (P)" value={`Rs.${summary.advanceAmount.toLocaleString("en-IN")}`} color="#e65100" />
//                   )}
...
//                     <span style={{ fontSize: "18px" }}>⚡</span>
//                     <span style={{ color: "#e65100", fontWeight: "bold", fontSize: "14px" }}>
//                       Advance Payment of Rs.{summary.advanceAmount.toLocaleString("en-IN")} already adjusted (P column mein jaayega)
//                     </span>
//                   </div>
//                 )}
...
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Net Amount:</span>
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#1976d2", fontSize: "16px" }}>
//                             Rs.{summary.netAmount.toLocaleString("en-IN")}
//                           </span>
//                         </div>
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>Previously Paid (H):</span>
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#388e3c", fontSize: "16px" }}>
...
//                         {summary.advanceAmount > 0 && (
//                           <div>
//                             <span style={{ color: "#555", fontWeight: "bold" }}>Advance (P column):</span>
//                             <span style={{ marginLeft: "10px", fontWeight: "bold", color: "#e65100", fontSize: "16px" }}>
...
//                         </div>
//                         <div>
//                           <span style={{ color: "#555", fontWeight: "bold" }}>New Balance (I):</span>
//                           <span style={{ marginLeft: "10px", fontWeight: "bold", color: summary.newBalance === 0 ? "#388e3c" : "#d32f2f", fontSize: "16px" }}>
...
//                       <div>
//                         <label style={{ fontSize: "13px", fontWeight: "bold", color: "#555", display: "block" }}>
//                           CURRENT PAID AMOUNT Rs. (H column)
//                         </label>
...
//                 <span style={{ color: "#856404" }}>
//                   H = Current Paid (input only) • P = Advance Amount • I = 0 if H+P = Net Amount, else remaining balance
//                 </span>
//               </div>
...
const Payment = () => {
  ...
  // vendorSearch aur siteSearch ko ref mein store karo
  // taaki handleFilter async context mein bhi latest value use kare
  const vendorSearchRef = useRef(vendorSearch);
  const siteSearchRef   = useRef(siteSearch);

  useEffect(() => {
    vendorSearchRef.current = vendorSearch;
  }, [vendorSearch]);

  useEffect(() => {
    siteSearchRef.current = siteSearch;
  }, [siteSearch]);
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Fresh processed data return karo
  ...
  // freshData optional param - submit ke baad use hoga
  const handleFilter = (freshData = null) => {
    ...
    const results = sourceData.filter(
      (item) =>
        ...
        (currentSite
          ? item.siteName.toLowerCase() === currentSite.toLowerCase()
          : true)
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = selectedBills.map((uid) => {
        ...
        const previousPaid  = summary.previousPaid;
        const advanceAmt    = summary.advanceAmount;
        const effectivePaid = summary.effectivePaid;
      ...
      if (response.data.success) {
        ...
        // Fresh data fetch karo aur automatically filter apply karo
        const freshData = await fetchInitialData();
        if (freshData) {
          handleFilter(freshData); // Auto refresh - page reload nahi chahiye
        }
      } else {
        alert("Error: " + (response.data.error || "Something went wrong"));
      }
    ...
    <div style={{ padding: "20px", backgroundColor: "#f0f2f5", minHeight: "100vh", fontFamily: "sans-serif" }}>
      ...
      <div style={{ backgroundColor: "white", padding: "25px", borderRadius: "12px", boxShadow: "0 4px 12px rgba(0,0,0,0.05)", marginBottom: "25px" }}>
        <div style={{ display: "flex", gap: "20px", alignItems: "flex-end" }}>
          ...
          {/* Vendor */}
          <div style={{ flex: 1, position: "relative" }} ref={vendorRef}>
            <label style={{ fontSize: "13px", fontWeight: "bold", color: "#555", marginBottom: "8px", display: "block" }}>
              Viewing Bills for:
            </label>
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
              <input type="text" placeholder="Search Vendor..." value={vendorSearch}
      ...
      {showData && (
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            ...
            <div
              style={{ color: "#d32f2f", fontWeight: "bold", cursor: "pointer", fontSize: "16px" }}
              onClick={() => { setShowData(false); setSelectedBills([]); setPaidAmounts({}); setRoundups({}); }}>
              <FaTimes style={{ marginRight: "8px" }} /> Change Contractor
            </div>
          </div>
