...
const RequirementReceived = () => {
  ...
  // ✅ Check karta hai ki form submit ke liye ready hai ya nahi
  const isFormValid = () => {
    // Basic fields check
    if (!formData.siteName || !formData.supervisorName || !formData.remark) {
      return false;
    }

    // Har item ka validation
  ...
  const handleSubmit = async (e) => {
    ...
    // Double-check (button disabled hone ke bawajood safety ke liye)
  ...
  return (
    <div className="mt-6 max-w-7xl mx-auto">
      <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            return (
              <div key={idx} className="border p-4 mb-4 rounded bg-gray-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">
                      Use For Reason <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {!formIsValid && (
          <p className="text-sm text-red-500 mb-3 text-right">
            * Saare required fields fill karein tab Submit button active hoga.
          </p>
        )}
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            title={!formIsValid ? "Saare required fields fill karein" : ""}
            className={`flex items-center gap-2 px-5 py-2 rounded-md text-white transition-all duration-200
              ${formIsValid && !loading
