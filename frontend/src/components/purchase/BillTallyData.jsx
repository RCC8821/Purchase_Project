...
//   ---------------------------------------------------- */
//   const fetchBillTallyData = async () => {
//     try {
//       setLoading(true);
...
//           ...new Set(result.data.map((item) => item.invoice13).filter(Boolean)),
//         ];
...
//       if (vendorsForInvoice.length === 1) setSelectedVendor(vendorsForInvoice[0]);
//     }
//   };
...
//   const handleNext = () => {
//     const newErrors = {};
//     if (!selectedInvoice) newErrors.invoice = 'Invoice 13 is required';
...
//     const itemsTotal = updatedItems.reduce((sum, i) => sum + parseFloat(i.total), 0);
...
//     if (!formData.status16) newErrors.status16 = 'Status 16 is required';
//     if (!formData.billNo) newErrors.billNo = 'Bill No is required';
//     if (!formData.billDate) newErrors.billDate = 'Bill Date is required';
//     if (!formData.transportWOGST) newErrors.transportWOGST = 'Transport amount is required';
//     if (!formData.gstRate) newErrors.gstRate = 'GST % is required';
...
//     if (!hasValidItem) newErrors.items = 'At least one item must have amount';
...
//         .toFixed(2);
//       const totalCGSTAmt = formData.items
//         .reduce((s, i) => s + (parseFloat(i.cgstAmt) || 0), 0)
//         .toFixed(2);
//       const totalSGSTAmt = formData.items
//         .reduce((s, i) => s + (parseFloat(i.sgstAmt) || 0), 0)
//         .toFixed(2);
//       const totalIGSTAmt = formData.items
//         .reduce((s, i) => s + (parseFloat(i.igstAmt) || 0), 0)
//         .toFixed(2);
...
//         items: [...itemsWithData, totalRow],

//         totalBillAmount: totalAmount,
//         totalCGST: totalCGSTAmt,
//         totalSGST: totalSGSTAmt,
//         totalIGST: totalIGSTAmt,
//         transportGstOnly,
//       };
...
//         btn.className =
//           'px-6 py-2 bg-blue-600 text-white rounded font-medium hover:bg-blue-700 shadow-md';
//         setIsSubmitting(false);
//       }, 2000);
//     }
//   };
...
//     !formData.gstRate ||
//     !formData.remark ||
//     formData.items.every((i) => !parseFloat(i.amount));
...
//               <tr>
//                 <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border-r border-gray-300">
//                   Planned 16
//                 </th>
//                 <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border-r border-gray-300">
//                   UID
...
//                   </td>
//                   <td className="px-4 py-2 text-sm text-gray-800 border-r border-gray-200 text-right">
//                     {item.revisedQuantity}
//                   </td>
//                   <td className="px-4 py-2 text-sm text-gray-800 border-r border-gray-200 text-right">
//                     {item.finalReceivedQuantity}
//                   </td>
//                   <td className="px-4 py-2 text-sm text-gray-800 border-r border-gray-200">
//                     {item.unitName}
//                   </td>
...
//                   </td>
//                   <td className="px-4 py-2 text-sm text-gray-800 border-r border-gray-200">
//                     {item.approvalQuotationNo}
//                   </td>
//                   <td className="px-4 py-2 text-sm text-gray-800 border-r border-gray-200">
//                     {item.approvalQuotationPDF ? (
//                       <a
//                         href={item.approvalQuotationPDF}
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="text-blue-600 underline"
//                       >
...
//                         errors.invoice ? 'border-red-500' : 'border-purple-300'
//                       }`}
//                     >
//                       <option value="">-- Select Invoice 13 --</option>
//                       {invoiceNumbers.map((inv, idx) => (
//                         <option key={idx} value={inv}>
//                           {inv}
//                         </option>
//                       ))}
//                     </select>
...
//                           readOnly
//                           className="mt-1 block w-full border-purple-300 rounded-md p-2 bg-gray-100"
//                         />
//                       ) : (
//                         <select
//                           value={selectedVendor}
...
//                           className={`mt-1 block w-full border rounded-md p-2 ${
//                             errors.vendor ? 'border-red-500' : 'border-purple-300'
//                           }`}
//                         >
...
//                         className={`mt-1 block w-full border rounded-md p-2 ${
//                           errors.status16 ? 'border-red-500' : 'border-gray-300'
//                         }`}
//                       >
...
//                       </select>
//                       {errors.status16 && <p className="text-red-500 text-xs mt-1">{errors.status16}</p>}
//                     </div>
...
//                             <tr>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-16">
//                                 UID
//                               </th>
//                               <th className="px-4 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300">
//                                 Material Name
//                               </th>
//                               <th className="px-4 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-32">
//                                 Amount *
//                               </th>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-20">
//                                 CGST
//                               </th>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-20">
//                                 IGST
//                               </th>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-24">
//                                 CGST Amt
//                               </th>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 w-24">
//                                 SGST Amt
...
//                                 IGST Amt
//                               </th>
//                               <th className="px-3 py-2 text-xs font-medium text-gray-700 text-center w-24">
//                                 Total
//                               </th>
//                             </tr>
//                           </thead>
...
//                                     value={item.uid}
//                                     readOnly
//                                     className="w-full text-center border-0 bg-transparent text-sm p-0"
//                                   />
//                                 </td>
...
//                                     readOnly
//                                     className="w-full bg-transparent border-0 text-sm p-0"
...
//                                     value={item.cgstAmt}
//                                     readOnly
//                                     className="w-full text-center bg-transparent border-0 text-sm p-0"
//                                   />
//                                 </td>
//                                 <td className="px-3 py-2 border-r border-gray-300 text-center">
//                                   <input
...
//                                     value={item.total}
//                                     readOnly
//                                     className="w-full text-center bg-transparent border-0 text-sm p-0 font-medium"
//                                   />
//                                 </td>
//                               </tr>
//                             ))}
...
//                             <tr>
//                               <td colSpan="2" className="px-3 py-2 text-right border-r border-gray-300">
//                                 Total
//                               </td>
...
//                           onChange={handleInputChange}
//                           className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 text-base ${
//                             errors.transportWOGST ? 'border-red-500' : 'border-gray-300'
//                           }`}
//                           placeholder="0.00"
//                           step="0.01"
//                         />
//                         {errors.transportWOGST && (
//                           <p className="text-red-500 text-xs mt-1">{errors.transportWOGST}</p>
//                         )}
//                       </div>
...
//                             (parseFloat(formData.gstRate) || 0) / 100
//                           ).toFixed(2)}
//                           readOnly
...
//                     className={`px-6 py-2 rounded font-medium shadow-md transition-all duration-300 ${
//                       isSubmitDisabled
...
const BillTallyData = () => {
  ...
  /* ----------------------------------------------------
     VALIDATE BEFORE SUBMIT - Only Required Fields
  ---------------------------------------------------- */
  const validateForm = () => {
    const newErrors = {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitData = {
        ...
        billNo: formData.billNo,
        billDate: formData.billDate,
        transportWOGST: formData.transportWOGST || '0',
            ...
            {step === 2 && (
              <div>
                ...
                <div className="space-y-4">
                  ...
                  {/* TRANSPORT SECTION - * REMOVED FROM LABELS */}
                  <div className="mt-6 border-t pt-6 bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-xl shadow-sm">
                    <div className="grid grid-cols-3 gap-6 mb-6">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
