...
const Dashboard = () => {
  ...
  const pageContent = {
    ...
    'Vendor_followup_billing': { title: 'Vendor_followup_billing', content: 'Vendor follow up billing management.' },
    'Bill_Processing': { title: 'Bill_Processing', content: 'Bill processing management.' },
    'BillCheckedData': { title: 'BillCheckedData', content: 'Bill checked data management.' },
  ...
  const getActiveMenuPages = () => {
    for (const menu of menuItems) {
      if (menu.pages && menu.pages.some(p => p.id === selectedPage)) {
        return menu.pages;
      }
    }
    return getPurchasePages();
  };
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50">
                ...
                <li key={menu.id} className="relative">
                  ...
                  >
                    ...
                    {menu.pages && menu.pages.length > 0 && isSidebarExpanded && (
                      <ChevronDown className={`w-4 h-4 ml-auto transition-transform duration-200 ${isDropdownOpen(menu) ? 'rotate-180' : ''}`} />
                    )}
                  </button>

                  {isSidebarExpanded && menu.pages && menu.pages.length > 0 && isDropdownOpen(menu) && (
                    <ul className="ml-6 mt-2 space-y-1 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg shadow-md max-h-60 overflow-y-auto">
                      {menu.pages.map((page) => (
                        <li key={page.id}>
                          <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="lg:hidden p-4 bg-white/80 backdrop-blur-sm border-b border-indigo-100">
          <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 rounded-lg hover:bg-indigo-50">
            <Menu className="w-6 h-6 text-indigo-700" />
          </button>
        </div>

        <main className="flex-1 overflow-auto p-4 lg:p-6">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 lg:p-6 shadow-lg border border-indigo-100 h-full">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 pb-4 border-b border-indigo-100">
              <div>
              ...
              {getActiveMenuPages().length > 0 && isPageAllowed() && (
                ...
                <div className="flex items-center justify-center h-64 bg-gradient-to-br from-pink-50 to-indigo-50 rounded-xl border-2 border-dashed border-indigo-200">
                  <div className="text-center">
                    ...
                    <p className="text-indigo-700 text-lg font-medium">
                      {selectedPage.startsWith('labour-') && !getCurrentComponent() 
