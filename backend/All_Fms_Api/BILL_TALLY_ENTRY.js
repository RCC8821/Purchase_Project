...
router.post('/bill_tally_entry', async (req, res) => {
  ...
  // ✅ Helper: value se leading/trailing quote aur spaces hatao
  ...
  try {
    ...
    data.items.forEach((item) => {
      ...
      if (rowIndex !== -1 || isTotalRow) {
        ...
        if (!isTotalRow) {
          // ✅ Normal item rows - cleanValue applied
          addUpdate(COL.STATUS_16, cleanValue(data.status16), rowNumber);
          ...
          addUpdate(
            COL.IGST_PERCENT,
            cleanValue(item.igst !== '0' ? item.igst : '0'),
            rowNumber
          );
    ...
    if (lastValidRow) {
      const row = lastValidRow.rowNumber;
      requests.push({
        range: `Billing_FMS!${COL.TRANSPORT_WO_GST}${row}`,
        values: [[cleanValue(data.transportWOGST)]],
      });
      requests.push({
        range: `Billing_FMS!${COL.NET_TRANSPORT}${row}`,
        values: [[transportGstOnly]],
      });
      requests.push({
        range: `Billing_FMS!${COL.GRAND_TOTAL}${row}`,
        values: [[grandTotal]],
      });
    }
    ...
    // ✅ Execute batch update - USER_ENTERED use karo
    if (requests.length > 0) {
      await sheets.spreadsheets.values.batchUpdate({
        spreadsheetId,
        resource: {
          valueInputOption: 'USER_ENTERED', // ✅ RAW ki jagah USER_ENTERED
          data: requests,
        },
      });
    }
