...
router.get('/get-vendor-follow-up-material', async (req, res) => {
  try {
    ...
    const headers = [
      ...
      { key: 'ACTUAL_8', column: 64 }, // BN
      { key: 'STATUS_8', column: 65 }, // BO
      { key: 'FOLLOW-UP_COUNT_8', column: 66 }, // BP
      { key: 'EXPECTED_DELIVERY_DATE_8', column: 67 }, // BQ
      { key: 'REMARK_RECEIVED_VENDOR_8', column: 68 }, // BR
      { key: 'PDF_URL_5', column: 54 }, // AK
      { key: 'PDF_URL_7', column: 60 }, // AQ
      { key: 'Vendor_Firm_Name_5', column: 38 }, // AM
      { key: 'Vendor_Contact', column: 40 }, // AN
    ...
    // Find ACTUAL_8, PLANNED_8, and STATUS_8 column indices dynamically
    const actual8Index = headers.find(h => h.key === 'ACTUAL_8')?.column;
    const planned8Index = headers.find(h => h.key === 'PLANNED_8')?.column;
    const status8Index = headers.find(h => h.key === 'STATUS_8')?.column;
    if (actual8Index === undefined || planned8Index === undefined || status8Index === undefined) {
      console.error('Required columns not found in headers');
      return res.status(500).json({ error: 'Invalid sheet structure', details: 'ACTUAL_8, PLANNED_8, or STATUS_8 column missing' });
    }
    ...
    const formData = dataRows
      .map((row, index) => {
        ...
        // Check if ACTUAL_8 is empty and PLANNED_8 is non-empty, or STATUS_8 is "Partition"
        const actual8 = row[actual8Index]?.trim() || '';
        const planned8 = row[planned8Index]?.trim() || '';
        const status8 = row[status8Index]?.trim() || '';
        console.log(
          `Row ${index + 8} - ACTUAL_8: "${actual8}", PLANNED_8: "${planned8}", STATUS_8: "${status8}", Full row: ${JSON.stringify(row)}`
        );

        const isNotDispatched = !actual8 && planned8;
        const isPartition = status8.toLowerCase() === 'partition';

        if (!isNotDispatched && !isPartition) {
          console.log(
            `Skipping row ${index + 8} - Reason: Not (ACTUAL_8 empty and PLANNED_8 non-empty) and STATUS_8 is not "Partition"`
          );
          return null;
        }
    ...
    console.log(`Rows with (ACTUAL_8 empty and PLANNED_8 non-empty) or STATUS_8 "Partition": ${validRowCount}`);
    console.log('Final formData:', JSON.stringify(formData, null, 2));
    ...
    if (!formData.length) {
      ...
      return res.status(404).json({
        error: 'No valid data found after filtering',
        details: validRowCount === 0
          ? 'No rows have ACTUAL_8 empty and PLANNED_8 non-empty, or STATUS_8 as "Partition" in columns BN, BM, and BO'
          : 'All rows with the criteria are empty in other columns',
      });
    }
