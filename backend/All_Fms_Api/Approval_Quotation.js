...
//     // Clean vendor and quotation details
//     const vendorName = cleanText(approvedItems[0]?.rowData[7]); // Vendor_Name
//     const vendorAddress = cleanText(approvedItems[0]?.rowData[8]); // Vendor_Address
//     const vendorGST = cleanText(approvedItems[0]?.rowData[10]); // Vendor_GST_No
//     const cleanQuotationNo = cleanText(quotationNo);
//     const cleanIndentNo = cleanText(indentNo);
...
//     const uidColumnIndex = 2; // Column C (UID)
//     const indentNoIndex = 4; // Column E (Indent_No)
//     const vendorFirmNameIndex = 7; // Column I (Vendor_Ferm_Name)
//     const approvalStatusIndex = 31; // Column AF (Approval Status)
//     const approvedQuotationNoIndex = 32; // Column AG (Approved Quotation No)
//     const pdfUrlIndex = 33; // Column AH (PDF URL)
...
//     // Track unique Indent_No and their corresponding approvedQuotationNo and approved vendors
//     const indentNoMap = new Map();
//     const approvedVendorsByIndent = new Map(); // indentNo -> set of approved vendor_firm_name
//     let approvedRowsData = [];

//     // First pass: Identify approved rows and collect approved vendors per indent
//     rows.forEach((row, index) => {
//       if (index === 0) return; // Skip header
//       const uid = row[uidColumnIndex]?.toString().trim() || '';
//       const vendorFirmName = row[vendorFirmNameIndex]?.toString().trim() || '';
...
//       if (matchingApproval && isApprovedStatus) {
//         if (!indentNoMap.has(indentNo)) {
//           approvalCounter++;
//           indentNoMap.set(indentNo, generateApprovedQuotationNo(approvalCounter));
//         }
//         if (!approvedVendorsByIndent.has(indentNo)) {
//           approvedVendorsByIndent.set(indentNo, new Set());
//         }
//         approvedVendorsByIndent.get(indentNo).add(vendorFirmName);
//         approvedRowsData.push({
//           uid,
//           vendor_firm_name: vendorFirmName,
//           indentNo,
...
//         });
//         console.log(
//           `Collected approved row for UID ${uid}, Vendor ${vendorFirmName}, Indent ${indentNo}`
//         );
//       }
//     });
//     console.log(
...
//     const approvedIndentNos = new Set(indentNoMap.keys());

//     // PDF Generation
//     let pdfUrlsByIndent = {};
//     let pdfErrors = [];
//     let indentGroups = {};
//     if (isApprovedStatus && approvedRowsData.length > 0) {
//       console.log('=== STARTING PDF GENERATION ===');
//       approvedRowsData.forEach((item) => {
//         if (!indentGroups[item.indentNo]) {
//           indentGroups[item.indentNo] = [];
...
//     // Second pass: Update rows in Quotation_Master
//     let updates = [];
//     let updatedCount = 0;
//     rows.forEach((row, index) => {
//       if (index === 0) return; // Skip header
...
//       const isMatching = !!matchingApproval;

//       let shouldUpdate = false;
//       let newStatus = '';

//       if (isApprovedStatus && approvedIndentNos.has(indentNo)) {
//         shouldUpdate = true;
//         newStatus = isMatching ? 'Approved' : 'Rejected';
//       } else if (!isApprovedStatus && isMatching) {
//         shouldUpdate = true;
//         newStatus = 'Rejected';
//       }
...
//         if (isApprovedStatus && isMatching) {
//           const quotationNo = indentNoMap.get(indentNo) || '';
//           updates.push({
//             range: `Quotation_Master!AG${index + 1}`,
//             values: [[quotationNo]],
...
//       console.error('No matching quotations found');
//       return res.status(404).json({
//         error: `No matching quotations found. Provided: ${JSON.stringify(approvals)}.`,
//       });
//     }

//     if (isApprovedStatus && indentNoMap.size > 0) {
//       console.log('Updating approval counter:', approvalCounter);
//       await sheets.spreadsheets.values.update({
//         spreadsheetId: process.env.SPREADSHEET_ID || spreadsheetId,
//         range: 'Quotation_Master!AM1',
...
//     // Update Purchase_FMS if approved
//     let purchaseUpdatedCount = 0;
//     if (isApprovedStatus && approvedRowsData.length > 0) {
//       console.log('=== UPDATING PURCHASE_FMS SHEET ===');
//       const purchaseRange = 'Purchase_FMS!B7:BE'; // Fetch from B7 to BE (UID to REMARK 5)
...
//         console.error('No data in Purchase_FMS sheet');
//       } else {
//         const purchaseData = purchaseRows.slice(1); // Skip header
//         let purchaseUpdates = [];

//         approvedRowsData.forEach((approvedItem) => {
//           const uid = approvedItem.uid;
//           const dataIndex = purchaseData.findIndex((pRow) => pRow[0]?.trim() === uid);
//           if (dataIndex === -1) {
//             console.error(`No matching UID ${uid} in Purchase_FMS`);
//             return;
//           }
//           const sheetRow = 8 + dataIndex; // Data starts at row 8
//           const rowData = approvedItem.rowData;
//           const pdfUrl = pdfUrlsByIndent[approvedItem.indentNo] || '';
//           const step5Values = [
//             approvedItem.quotationNo, // QUOTATION No. 5 (AL)
//             rowData[6]?.trim() || '', // Vendor Name 5 (AM)
//             rowData[7]?.trim() || '', // Vendor Firm Name 5 (AN)
//             rowData[8]?.trim() || '', // Vendor Address 5 (AO)
//             rowData[9]?.trim() || '', // Contact 5 (AP)
//             rowData[10]?.trim() || '', // Vendor GST No. 5 (AQ)
//             rowData[11]?.trim() || '', // Rate 5 (AR)
//             rowData[12]?.trim() || '', // Discount (AS)
//             rowData[13]?.trim() || '', // CGST 5 (AT)
//             rowData[14]?.trim() || '', // SGST 5 (AU)
//             rowData[15]?.trim() || '', // IGST 5 (AV)
//             rowData[16]?.trim() || '', // FINAL RATE 5 (AW)
//             rowData[30]?.trim() || '', // TOTAL VALUE 5 (AX)
//             'Approved', // APPROVAL 5 (AY)
//             rowData[21]?.trim() || '', // IS TRANSPORT REQUIRED 5 (AZ)
//             rowData[22]?.trim() || '', // EXPECTED TRANSPORT CHARGES 5 (BA)
//             rowData[23]?.trim() || '', // FRIGHET CHARGES 5 (BB)
//             rowData[24]?.trim() || '', // EXPECTED FRIGHET CHARGES 5 (BC)
//             pdfUrl, // PDF URL 5 (BD)
//             rowData[27]?.trim() || '', // REMARK 5 (BE)
//           ];

//           purchaseUpdates.push({
//             range: `Purchase_FMS!AL${sheetRow}:BE${sheetRow}`,
//             values: [step5Values],
//           });
...
//             },
//           });
//           console.log(`Batch updated ${purchaseUpdates.length} cells in Purchase_FMS`);
//         }
//       }
//     }
...
//       message: `Updated ${updatedCount} rows. Generated PDFs: ${Object.keys(
//         pdfUrlsByIndent
//       ).length}/${Object.keys(indentGroups).length}${isApprovedStatus ? `. Updated ${purchaseUpdatedCount} rows in Purchase_FMS` : ''}`,
//       pdfUrls: pdfUrlsByIndent,
//       errors: pdfErrors.length > 0 ? pdfErrors : undefined,
//       indentNosProcessed: Array.from(indentNoMap.keys()),
//     };
...
// Define generateQuotationPDF
const generateQuotationPDF = async (approvedItems, quotationNo, indentNo) => {
  console.log(`Generating PDF: Quotation ${quotationNo}, Indent ${indentNo}, Items: ${approvedItems.length}`);
  try {
    // Fetch units from Purchase_FMS using UIDs
    const unitMap = new Map();
    const fmsResponse = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.SPREADSHEET_ID || spreadsheetId,
      range: 'Purchase_FMS!B7:J', // B (UID) to J (Unit_Name), starting from row 7
    });
    const fmsRows = fmsResponse.data.values || [];
    fmsRows.slice(1).forEach(row => { // Skip header row
      const uid = row[0]?.trim();
      const unit = row[8]?.trim() || ''; // Unit_Name at index 8
    ...
    // Clean vendor and quotation details
    const vendorName = cleanText(approvedItems[0]?.rowData[7]); // Vendor_Ferm_Name
    const vendorAddress = cleanText(approvedItems[0]?.rowData[8]); // Vendor_Address
    const vendorGST = cleanText(approvedItems[0]?.rowData[10]); // Vendor_GST_No
    const cleanQuotationNo = cleanText(quotationNo);
    const cleanIndentNo = cleanText(indentNo);
    ...
    const tableBody = approvedItems.map((item, index) => {
      ...
      return [
        ...
        cleanText(row[5]), // Material_name
        cleanText(row[29]), // Total_Quantity
        cleanText(unitMap.get(uid) || ''), // Unit_Name from Purchase_FMS
        cleanText(row[11]), // RATE
        cleanText(row[13]), // CGST
        cleanText(row[14]), // SGST
        cleanText(row[15]), // IGST
  ...
  try {
    ...
    if (isApprovedStatus && approvedRowsData.length > 0) {
      ...
      for (const [indentNo, group] of Object.entries(indentGroups)) {
        ...
        try {
          const quotationNo = indentNoMap.get(indentNo);
          const pdfDataUri = await generateQuotationPDF(group, quotationNo, indentNo);
          console.log(`PDF data URI length for ${indentNo}: ${pdfDataUri.length}`);
