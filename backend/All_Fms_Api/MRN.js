  ...
  try {
    ...
    const filteredData = data
      ...
      .map(row => ({
        ...
        unitName: row[9] || '',
        purpose: row[10] || '',
        contractorName: row[12] || '',  // Column M - Contractor Name
        pdfUrl3: row[25] || '',
        pdfUrl5: row[55] || '',
        pdfUrl7: row[61] || '',
        finalReceivedQuantity9: row[75] || '',
...
// PDF Generation Function for MRN - WITH CONTRACTOR NAME
const generateMRNPDF = (rowDatas, mrnNo, poNumber, indentNo, deliveryDate) => {
  const doc = new jsPDF();
  if (typeof doc.autoTable !== 'function') {
    throw new Error('autoTable plugin not loaded');
  ...
  const pageHeight = doc.internal.pageSize.getHeight();
  const margin = 15;
  const tableStartY = 115; // Increased to accommodate contractor name
  ...
  const vendorName = cleanText(rowDatas[0]?.vendorName || '-');
  const siteName = cleanText(rowDatas[0]?.siteName || '-', true);
  const contractorName = cleanText(rowDatas[0]?.contractorName || '-'); // NEW: Contractor Name
  const pdfDeliveryDate = cleanText(deliveryDate || '-');

  // Row 1: MRN Number & MRN Date
  doc.setFont('helvetica', 'bold');
  doc.text('MRN Number:', margin, infoY);
  doc.setFont('helvetica', 'normal');
  doc.text(cleanText(mrnNo), margin + 35, infoY);
  ...
  // Row 4: Delivery Date & Contractor Name (NEW)
  doc.setFont('helvetica', 'bold');
  doc.text('Delivery Date:', margin, infoY + 24);
  doc.setFont('helvetica', 'normal');
  doc.text(pdfDeliveryDate, margin + 35, infoY + 24);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Contractor Name:', pageWidth / 2 + 15, infoY + 24);
  doc.setFont('helvetica', 'normal');
  doc.text(contractorName, pageWidth / 2 + 50, infoY + 24);

  // Material Details Header
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(220, 53, 69);
  doc.text('Material Details', margin, infoY + 48);

  // Table Body - WITH CONTRACTOR NAME COLUMN
  const tableBody = rowDatas.map((rowData, index) => [
    index + 1,
    cleanText(rowData.uid),
    cleanText(rowData.materialType),
    ...
    cleanText(rowData.vehicleNo),
    cleanText(rowData.contractorName), // NEW: Added Contractor Name in table
  ]);

  doc.autoTable({
    head: [['Sr No.', 'UID', 'Material Type', 'SKU', 'Material Name', 'Unit', 'Brand', 'Final Qty', 'Challan No.', 'Vehicle No.', 'Contractor']],
    body: tableBody,
    startY: tableStartY,
    theme: 'grid',
    styles: {
      fontSize: 6.5, // Slightly smaller to fit new column
      cellPadding: 2,
      font: 'helvetica',
      textColor: [0, 0, 0],
      lineColor: [200, 200, 200],
    ...
    columnStyles: {
      ...
      1: { cellWidth: 15 },  // UID
      2: { cellWidth: 18 },  // Material Type
      3: { cellWidth: 15 },  // SKU
      4: { cellWidth: 25 },  // Material Name
      5: { cellWidth: 10 },  // Unit
      6: { cellWidth: 15 },  // Brand
      7: { cellWidth: 15 },  // Final Qty
      8: { cellWidth: 20 },  // Challan No
      9: { cellWidth: 18 },  // Vehicle No
      10: { cellWidth: 20 }, // Contractor Name (NEW)
    },
    margin: { top: tableStartY, left: margin, right: margin },
    tableWidth: pageWidth - (2 * margin),
    pageBreak: 'auto',
...
router.post('/save-MRN-data', async (req, res) => {
  ...
  try {
    ...
    for (let uidIndex = 0; uidIndex < purchaseFmsUIDs.length; uidIndex++) {
      ...
      // rowData WITH CONTRACTOR NAME (Column M = index 12)
      const rowData = {
        uid: row[1] || '-',
        siteName: row[3] || '-',
        vendorName: row[39] || '-',
        contractorName: row[12] || '-',  // Column M - Contractor Name (NEW)
        materialType: row[5] || '-',
        skuCode: row[6] || '-',
        materialName: row[7] || '-',
        unit: row[9] || '-',
        brand: row[17] || '-',
        finalReceivedQuantity: finalReceivedQuantity || row[75] || '-',
        challanNo: challanNos || (challanNo || '-'),
        vehicleNo: vehicleNo || row[78] || '-',
        deliveryDate: row[62] || deliveryDate || '-',
      };
      console.log(`Row data for UID ${purchaseFmsUID}:`, rowData);
